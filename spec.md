...
10. **Vendor surface — no portal.** Tokenized reply-to RFQ emails (`rfq+<token>@`) so vendors reply as plain email and the agent threads + parses; optional magic-link quote form per RFQ (no accounts); fiscal self-onboarding magic link (CSF / 32-D upload, 69-B check, expiry reminders). Design rule: vendor-facing surface only when it reduces vendor friction below email — the north-star metric is RFQ response rate. A full accounts-based portal is gated on a recurrence signal (same vendor responding >10 RFQs/quarter), never built speculatively.
11. **CFDI reception channels (MX):** primary = SAT descarga masiva via wrapper API (Syntage/SW Sapien-class) under the tenant's e.firma — vault-grade custody, never stored plain; fallback = dedicated `facturas@` inbox parsing XML attachments. Both feed `CfdiInvoiceAdapter` → deterministic 3-way match (issuer RFC ↔ vendors.tax_id, total ↔ PO, UUID + cancellation status ↔ SAT).
12. **Quality floor — qualify, then score.** Two-stage evaluation: binary qualification gate (spec compliance, category minimum warranty, vendor approved, tax status clean) before weighted scoring; category-specific weights; price-outlier flag (quote below baseline median −25% escalates to human instead of auto-winning); BAFO round (best-and-final-offer) before any live reverse auction, auctions only for commodity categories with 5+ qualified vendors.
13. **Outcome capture — ground truth loop.** One-click reception confirmation by the requester (on time? as specified?) + warranty-claim log feed vendor ratings deterministically; repeated failures auto-propose blocking with audit-cited cause. This per-tenant verified-performance dataset is the data moat.
14. **Research agent lane (read-only, human-gated).** Autonomous agents for vendor discovery/enrichment, external price benchmarking (kept separate from internal baselines, with provenance), and compliance monitoring — outputs land in staging tables as proposals; the deterministic core and humans dispose. Agents never touch policy, scoring, or the approval path.
